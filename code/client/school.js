...
Template.school.helpers({
  ...
  participants: function(){
      var donation = Donations.findOne({_id: Session.get("donateId")});

      if (donation) {
        var keyOrg = Organizations.findOne({createdBy: donation.organization});
...
Template.schools.events({
   ...
   'click #donateNow':function(event){
   	  ...
   	  // if(Meteor.userId() && this._id) {
   	  	var donation = Donations.findOne({
   	  		$or: [
              {$and: [{organization: Meteor.userId()}, {school: this._id}]}, 
              {$and: [{organization: Meteor.userId()}, {school: this._id}]}
          ]
   	  	});
        if(donation){
          ...
          var newDonation = Donations.findOne({
          $or: [
              {$and: [{organization: Meteor.userId()}, {school: this._id}]}, 
              {$and: [{organization: Meteor.userId()}, {school: this._id}]}
          ]
